...
<aside class="index_aside" style="background-color: lightgray; float: left; width: 15%; height: 100%; position: fixed; text-align: center; left: 0; top: 0";>
    ...
    <?php include "db_connect.php"; include "externalPHPfiles/userDAO.php"; include "externalPHPfiles/rank_selector.php";
...
<div class="container_push">
    <div style="position:relative; left: 400px; top: 175px; width: 720px; height: 512px; background-image: url('assets/trading-bg.png'); color: white">
        ...
        <form style="position:relative; left: 0; top: 130px; text-align: center;" action="trading_join.php" method="POST">
            <p>Írd be a csere másik tagja által kapott kódot!</p>
            <label for="code" hidden></label>
            <input id="code" type="text" name="code" style="font-size: 26px; text-align: center;" minlength="8" maxlength="8" size="8"><br>
            <input style="background-color: transparent; padding: 10px calc(455px / 3) 10px calc(455px / 3); border: 1px solid darkgray; color: white" type="submit" name="join_trade" value="Csatlakozás a cseréhez">
        </form>
    </div>
</div>
...
<?php
    if (isset($_POST["join_trade"])) {
        ...
        add_tradee($_SESSION["username"], $_POST["code"]);
        setcookie("trade-initiated", true);
        setcookie("tradee", true);
        setcookie("trade-code", $_POST["code"]);
